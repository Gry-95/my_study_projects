...
class MovieModelTestCase(TestCase):
    ...
    def test_del_object(self):
        Movie.objects.filter(name='Mask').delete()
        self.assertNotIn('Mask', Movie.objects.all())

    def test_changes_object(self):
        Movie.objects.create(name='Mortal Combat', rating=22)
        self.assertEqual(Movie.objects.get(name='Mortal Combat').name, 'Mortal Combat')
